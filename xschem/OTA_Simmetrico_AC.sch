v {xschem version=3.4.5 file_version=1.2
}
G {}
K {}
V {}
S {}
E {}
B 2 810 -250 1360 0 {flags=graph,unlocked
y1=-6.9
y2=41
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=0
x2=12

subdivx=8
node="\\"out db20()\\""
color=4
dataset=0
unitx=1
logx=1
logy=0
divx=5
sweep=""
rawfile=$netlist_dir/OTA_Simmetrico_AC.raw
sim_type=ac}
B 2 810 -820 1360 -500 {flags=graph,unlocked
y1=-0.00135896
y2=0.00142796
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=0
x2=6e-12




dataset=0
unitx=1
logx=0
logy=0
divx=5
hilight_wave=-1
rawfile=$netlist_dir/OTA_Simmetrico_AC.raw
sim_type=tran
subdivx=3}
B 2 810 -500 1360 -250 {flags=graph,unlocked
rawfile=$netlist_dir/OTA_Simmetrico_AC.raw
sim_type=ac
y1=0
y2=180
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=0
x2=12

subdivx=8
node=ph(out)
color=4
dataset=0
unitx=1
logx=1
logy=0
divx=5
sweep=""}
N -175 -282.5 -175 -222.5 {
lab=#net1}
N 25 -282.5 25 -222.5 {
lab=#net2}
N 65 -92.5 165 -92.5 {
lab=#net2}
N -335 -62.5 -335 -32.5 {
lab=GND}
N 195 -62.5 195 -32.5 {
lab=GND}
N -335 -32.5 205 -32.5 {
lab=GND}
N -295 -92.5 -215 -92.5 {
lab=#net1}
N -175 -62.5 -175 -32.5 {
lab=GND}
N 25 -62.5 25 -32.5 {
lab=GND}
N -245 -142.5 -245 -92.5 {
lab=#net1}
N -215 -142.5 -175 -142.5 {
lab=#net1}
N -245 -142.5 -215 -142.5 {
lab=#net1}
N 105 -142.5 105 -92.5 {
lab=#net2}
N 25 -142.5 65 -142.5 {
lab=#net2}
N 65 -142.5 105 -142.5 {
lab=#net2}
N -335 -402.5 -335 -222.5 {
lab=#net3}
N 195 -402.5 195 -222.5 {
lab=OUT}
N -295 -532.5 155 -532.5 {
lab=#net3}
N -335 -482.5 -265 -482.5 {
lab=#net3}
N -265 -532.5 -265 -502.5 {
lab=#net3}
N -335 -602.5 -335 -562.5 {
lab=VDD}
N -335 -602.5 195 -602.5 {
lab=VDD}
N 195 -602.5 195 -562.5 {
lab=VDD}
N -175 -392.5 -175 -342.5 {
lab=#net4}
N -175 -392.5 25 -392.5 {
lab=#net4}
N 25 -392.5 25 -342.5 {
lab=#net4}
N -335 -462.5 -335 -402.5 {
lab=#net3}
N 195 -462.5 195 -402.5 {
lab=OUT}
N -265 -502.5 -265 -482.5 {
lab=#net3}
N -75 -602.5 -75 -502.5 {
lab=VDD}
N -75 -402.5 -75 -392.5 {
lab=#net4}
N 205 -32.5 295 -32.5 {
lab=GND}
N -335 -502.5 -335 -462.5 {
lab=#net3}
N 195 -502.5 195 -462.5 {
lab=OUT}
N -175 -92.5 -105 -92.5 {
lab=GND}
N -105 -92.5 -105 -32.5 {
lab=GND}
N -45 -92.5 25 -92.5 {
lab=GND}
N -45 -92.5 -45 -32.5 {
lab=GND}
N 195 -92.5 265 -92.5 {
lab=GND}
N 265 -92.5 265 -32.5 {
lab=GND}
N -405 -92.5 -335 -92.5 {
lab=GND}
N -405 -92.5 -405 -32.5 {
lab=GND}
N -405 -532.5 -335 -532.5 {
lab=VDD}
N -405 -602.5 -405 -532.5 {
lab=VDD}
N 195 -532.5 265 -532.5 {
lab=VDD}
N 265 -602.5 265 -532.5 {
lab=VDD}
N 65 -312.5 95 -312.5 {
lab=IN2}
N -245 -312.5 -215 -312.5 {
lab=IN1}
N -175 -312.5 -105 -312.5 {
lab=#net4}
N -105 -372.5 -105 -312.5 {
lab=#net4}
N -175 -372.5 -105 -372.5 {
lab=#net4}
N -45 -312.5 25 -312.5 {
lab=#net4}
N -45 -372.5 -45 -312.5 {
lab=#net4}
N -45 -372.5 25 -372.5 {
lab=#net4}
N -175 -222.5 -175 -122.5 {
lab=#net1}
N 25 -222.5 25 -122.5 {
lab=#net2}
N 195 -162.5 195 -122.5 {
lab=#net5}
N -335 -162.5 -335 -122.5 {
lab=#net6}
N 195 -602.5 295 -602.5 {
lab=VDD}
N 295 -32.5 295 7.5 {
lab=GND}
N -735 -602.5 -735 -572.5 {
lab=VDD}
N -735 -512.5 -735 -482.5 {
lab=GND}
N -620 -512.5 -620 -492.5 {
lab=GND}
N -615 -327.5 -615 -312.5 {
lab=GND}
N -735 -327.5 -735 -312.5 {
lab=GND}
N -735 -407.5 -735 -387.5 {
lab=IN1}
N -615 -407.5 -615 -387.5 {
lab=IN2}
N 195 -402.5 325 -402.5 {
lab=OUT}
N -405 -32.5 -335 -32.5 {
lab=GND}
N -405 -602.5 -335 -602.5 {
lab=VDD}
N -75 -442.5 -75 -402.5 {
lab=#net4}
C {devices/iopin.sym} -735 -602.5 0 0 {name=p1 lab=VDD
}
C {sky130_fd_pr/pfet_01v8_hvt.sym} -195 -312.5 0 0 {name=M1
L=1
W=1
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8_hvt
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8_hvt.sym} 45 -312.5 0 1 {name=M2
L=1
W=1
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8_hvt
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8.sym} -195 -92.5 0 0 {name=M3
L=23
W=0.5
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8.sym} 45 -92.5 0 1 {name=M4
L=23
W=0.5
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8.sym} 175 -92.5 0 0 {name=M5
L=1
W=10
nf=10
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8.sym} -315 -92.5 0 1 {name=M6
L=1
W=10
nf=10
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8_hvt.sym} -315 -532.5 0 1 {name=M7
L=95
W=0.5
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8_hvt
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8_hvt.sym} 175 -532.5 0 0 {name=M8
L=95
W=0.5
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8_hvt
spiceprefix=X
}
C {devices/lab_pin.sym} 295 -602.5 0 1 {name=p6 sig_type=std_logic lab=VDD}
C {devices/ipin.sym} -245 -312.5 0 0 {name=p3 lab=IN1}
C {devices/ipin.sym} 95 -312.5 0 1 {name=p4 lab=IN2}
C {devices/opin.sym} 325 -402.5 0 0 {name=p5 lab=OUT}
C {devices/ammeter.sym} 195 -192.5 0 0 {name=Vmeas3 savecurrent=true}
C {devices/ammeter.sym} -335 -192.5 0 0 {name=Vmeas4 savecurrent=true}
C {sky130_fd_pr/corner.sym} -1185 -647.5 0 0 {name=CORNER only_toplevel=true corner=tt}
C {devices/lab_pin.sym} 295 7.5 0 0 {name=l1 sig_type=std_logic lab=GND}
C {devices/vsource.sym} -735 -542.5 0 0 {name=V3 value=1.8}
C {devices/lab_pin.sym} -735 -482.5 0 0 {name=l3 sig_type=std_logic lab=GND}
C {devices/gnd.sym} -620 -492.5 0 0 {name=l2 lab=GND}
C {devices/lab_pin.sym} -620 -512.5 0 0 {name=l4 sig_type=std_logic lab=GND}
C {devices/vsource.sym} -735 -357.5 0 0 {name=Vbias value="0 ac 1 0
+ sin(0 1m 1T 0 0 0)"}
C {devices/vsource.sym} -615 -357.5 0 0 {name=VbiasR value=0.9}
C {devices/lab_pin.sym} -735 -407.5 0 0 {name=l7 sig_type=std_logic lab=IN1
}
C {devices/lab_pin.sym} -615 -407.5 0 0 {name=l8 sig_type=std_logic lab=IN2
}
C {devices/launcher.sym} -985 -582.5 0 0 {name=h1
descr=Backannotate
tclcommand="xschem annotate_op $netlist_dir/[file tail [file rootname [xschem get current_name]]].raw"
}
C {devices/launcher.sym} -985 -627.5 0 0 {name=h2
descr="Annotate OP" 
tclcommand="set show_hidden_texts 1; xschem annotate_op"
}
C {devices/isource.sym} -75 -472.5 0 0 {name=I0 value=10u}
C {devices/simulator_commands_shown.sym} -1180 -470 0 0 {name=COMMANDS1
simulator=ngspice
only_toplevel=false 
value="
.option reltol=1e-5
+  abstol=1e-14 savecurrents
.control
  save all
  op
  remzerovec 
  write OTA_Simmetrico_AC.raw
  set appendwrite
  ac dec 10 1 1e12
  remzerovec
  write OTA_Simmetrico_AC.raw
  tran 1e-14 6e-12
  write OTA_Simmetrico_AC.raw
.endc
"}
C {devices/lab_pin.sym} -735 -312.5 0 0 {name=l5 sig_type=std_logic lab=GND}
C {devices/lab_pin.sym} -615 -312.5 0 0 {name=l6 sig_type=std_logic lab=GND}
C {devices/launcher.sym} 590 -180 0 0 {name=h3
descr="Netlist & sim" 
tclcommand="xschem netlist; xschem simulate"}
C {devices/launcher.sym} 590 -90 0 0 {name=h4 
descr="Load/unload
AC waveforms" 
tclcommand="
xschem raw_read $netlist_dir/OTA_Simmetrico_AC.raw ac
"
}
C {devices/launcher.sym} 590 -30 0 0 {name=h6 
descr="Load/unload
TRAN waveforms" 
tclcommand="
xschem raw_read $netlist_dir/OTA_Simmetrico_AC.raw tran
"
}
C {devices/launcher.sym} 590 -140 0 0 {name=h7
descr="Annotate OP" 
tclcommand="set show_hidden_texts 1; xschem annotate_op"
}
